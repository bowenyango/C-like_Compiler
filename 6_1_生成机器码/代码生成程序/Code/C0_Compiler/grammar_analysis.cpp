...
void C0_Compiler::ret_func_def(string* func_name) {
	...
	new_add_mid_code(FUNCBEGINOP, new string(), new string(), func_name);
	...
	if (current_token->t_type != IDENT)
		...
		if (current_token->t_type == BECOME) {
			...
			if (var_is_temp(temp_ident)) {
				//mid_code* temp_code = mid_code_table[mid_code_table_top - 1];
				//temp_code->des = sym_temp->name; // ????修改地址还是指针
				mid_code* temp_code;
				temp_code = get_mid_code(mid_code_table_top - 1);

				*temp_code->des = *sym_temp->name;
...
void C0_Compiler::condition(string* label_name) {
	...
	if (two_expression && exp_type_1 == exp_type_2) {
	...
	if (current_token->t_type != PRINTF_SYMBOL)
		...
		if (current_token->t_type != LEFT_PAR)
			...
			if (current_token->t_type == STRING) {
				...
				new_add_mid_code(PRINTFOP, string_index_str, new string("string"), string_index_str);
				...
				if (current_token->t_type != COMMA && current_token->t_type != RIGHT_PAR)
					...
					if (current_token->t_type != RIGHT_PAR)
						...
						switch (temp_ret) {
						case INTRET:
							new_add_mid_code(PRINTFOP, temp_ident, new string("int"), temp_ident);
							break;
						case CHARRET:
							new_add_mid_code(PRINTFOP, temp_ident, new string("char"), temp_ident);
							break;
						}
