...
void C0_Compiler::save_spot() {
	last_spot = infile.tellg();
	last_begin_pos = begin_pos;
	last_current_char = current_char;
	last_current_line = current_line;
	last_current_pos = current_pos;
	last_line = new_line;
	last_symbol = current_symbol_string;
	last_token = current_token;
	last_token_number = token_number;
}

void C0_Compiler::back_spot() {
	infile.seekg(last_spot);
	begin_pos = last_begin_pos;
	current_char = last_current_char;
	current_line = last_current_line;
	current_pos = last_current_pos;
	new_line = last_line;
	current_symbol_string = last_symbol;
	current_token = last_token;
	token_number = last_token_number;
}

void C0_Compiler::next_symbol() {
	static int tt = 0;
	if (current_line == 54 && tt == 0) {
		output_symtab_tofile(54);
		tt++;
	}

	if (current_char == '\0' && infile.eof() != true)
		next_line();
	...
	if (is_plus()) {
		...
		clear_token();
		do {
			current_symbol_string.push_back(current_char);
			next_letter();
		} while (is_char() || is_int());

		back_letter();

		std::transform(current_symbol_string.begin(), current_symbol_string.end(), current_symbol_string.begin(), ::tolower);
		...
		if (mark = is_symbol()) {
			...
			temp->t_value.pstring = new string(current_symbol_string);
			temp->t_value_type = PSTRING;
			addin_table();
			current_token = temp;
		}
		...
		cout << current_char << '\0' << endl;
	}

	////&*******************#$%^&*(
	//outfile << temp->line << "\t" << temp->pos << "\t" << token_type_enum_to_string[temp->t_type] << "\t";
...
void C0_Compiler::skip_spacetab() {
	while (is_space() || is_tab() || is_nextline() || current_char == '\0') {
		if (is_nextline() || current_char == '\0' && infile.eof() != true)
			next_line();
...
int C0_Compiler::is_symbol() {
	...
	for (i = 0; i <= WHILE_SYMBOL - CONST_SYMBOL; i++) {
		if (token_symbol[i] == current_symbol_string)
...
void C0_Compiler::ERROR(int err_typ) {
	...
	cout << " ***" << token_type_enum_to_string[current_token->t_type] << "*** ";
	errorfile << "ERROR HAPPEN IN " << current_line << "_" << begin_pos;
	errorfile << " ***" << token_type_enum_to_string[current_token->t_type] << "*** ";
	switch (err_typ) {
