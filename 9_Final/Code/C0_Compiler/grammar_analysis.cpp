	...
	if (current_token->t_type == IDENT) {
		...
		if (current_token->t_type == LEFT_PAR) {
			...
			*var_name = string("#RET");
			...
			if (var_is_number(array_index)) {
				symbol* temp = find_symbol(sym_name, current_func, not_localed);
				if (temp->number <= atoi(array_index->c_str()) || atoi(array_index->c_str()) < 0)
					ERROR(-1);
			}
...
void C0_Compiler::become_state() {
	...
	if (current_token->t_type != IDENT)
		...
		if (current_token->t_type == BECOME) {
			...
			if (var_is_number(temp_index)) {
				symbol* temp = find_symbol(sym_name, current_func, not_localed);
				if(temp->number <= atoi(temp_index->c_str()) || atoi(temp_index->c_str()) < 0)
					ERROR(-1);
			}
