...
bool C0_Compiler::new_add_mid_code(mid_op op, string* var_1, string* var_2, string* des) {
	...
	//	break;
	//case SCANFOP:
	//	cout << "scanf " << *var_2 << " " << *des << " " << endl; //*var_2是数据类型 *des是存储位置
	//	break;
	//case PRINTFOP:
	//	cout << "printf " << *var_2 << " " << *des << " " << endl; //*var_2是类型，*des是数据
	//	break;
	//case EXITOP:
	//	cout << "exit " << endl;
	//	break;
...
void C0_Compiler::push_string(string* string_val, int* string_index) {
	...
	*string_index = string_number;
	string_table[string_number] = string_val; //?为什么不可以
...
void C0_Compiler::output_mid_code_tofile(string path) { //暂时没用
	int i;
	mid_code* temp;
	mid_op op;
	...
	for (i = 0; i < mid_code_table_top; i++) {
		...
		case SCANFOP:
			...
			break;
		case PRINTFOP:
			midcodefile << "printf " << *var_2 << " " << *des << " " << *var_1 << endl; //*var_2是类型，*des是数据 *var_1换行
			break;
		case EXITOP:
			midcodefile << "exit " << endl;
			break;
