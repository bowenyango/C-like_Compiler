...
void C0_Compiler::output_mips_code(string path) {
	int i;
	objectcodefile.close();
	objectcodefile.open(path, ios::out);
...
void C0_Compiler::mips_func_init(string* func_name) { //
	...
	new_mips(ADDU_STR, FP_STR, R0_STR, SP_STR); // fp = sp;
	...
	//cout << *var_1 << " " << *var_2 << endl;
 	if (var_is_number(var_1)) {
		...
		if (var_is_number(var_2)) {
			...
			get_use_reg(var_2, reg_2);
			get_result_reg(des, reg_3);
			new_mips(ADDU_STR, reg_3, reg_1, reg_2);
		}
	}
	write_back(des, reg_3);
}
...
void C0_Compiler::mid_to_mips() {
	...
	new_mips(ADDU_STR, FP_STR, SP_STR, R0_STR);
...
void C0_Compiler::mips_generate(string path) {
	ascii_init();
	mid_to_mips();
	output_mips_code(path);
